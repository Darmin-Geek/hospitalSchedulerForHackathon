...
def schedule():
    ...
    # The maximum time is 48 hours * 60 minutes/hour = 2880 minutes
    ...
    for task in data['tasks']:
        for task_instance in range(task['number_of_times']):
            model.Add(sum(variables[(task['id'], task_instance, nurse['id'], time)] for nurse in data['nurses'] for time in range(0, 48*60, TIME_INTERVAL)) == 1)
                    ...
                    for task2 in data['tasks']:
                        for task2_instance in range(task2['number_of_times']):
                            for nurse2 in data['nurses']:
                                for time2 in range(0, 48*60, TIME_INTERVAL):
    ...
    model.maximize(sum(goal_variable_coefficients[((task['id'], task_instance, nurse['id'], time),(task2['id'], task2_instance, nurse2['id'], time2))] * goal_variables[((task['id'], task_instance, nurse['id'], time),(task2['id'], task2_instance, nurse2['id'], time2))] for task in data['tasks'] for task_instance in range(task['number_of_times']) for nurse in data['nurses'] for time in range(0, 48*60, TIME_INTERVAL) for task2 in data['tasks'] for task2_instance in range(task2['number_of_times']) for nurse2 in data['nurses'] for time2 in range(0, 48*60, TIME_INTERVAL) if (task['id'], task_instance, time) != (task2['id'], task2_instance,  time2)) \
